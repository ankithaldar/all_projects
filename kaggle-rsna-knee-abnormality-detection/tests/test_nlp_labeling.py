...
@pytest.mark.parametrize('report,target,expected', [
...
@pytest.mark.parametrize('report,target,expected', [
  ...
  ('Patellofemoral cartilage loss with joint space narrowing.',
   'PF OA', POSITIVE),
  ('Tricompartmental degenerative joint disease.',
   'Medial OA', POSITIVE),
...
def test_build_pseudo_labels_prefers_gold():
  ...
  from knee.helpers.nlp_labeling import build_pseudo_labels
  ...
  out = build_pseudo_labels(frame, 'StudyInstanceUID',
                            target_columns=['ACL'])
