...
class AsymmetricFocalLoss(nn.Module):
  ...
  def forward(self, logits: Tensor, targets: Tensor) -> Tensor:
    ...
    loss_pos = (
      -torch.pow(1.0 - torch.sigmoid(x_pos), self.gamma_pos)
      * torch.log(torch.sigmoid(x_pos) + 1e-8)
      * y
    )
    loss_neg = (
      -torch.pow(1.0 - torch.sigmoid(x_neg), self.gamma_neg)
      * torch.log(1.0 - torch.sigmoid(x_neg) + 1e-8)
      * (1.0 - y)
    )
