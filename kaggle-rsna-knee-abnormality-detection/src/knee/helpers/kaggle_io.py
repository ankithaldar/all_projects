...
class KaggleDatasetClient:
    ...
    def _run(self, args: list[str]) -> subprocess.CompletedProcess:
        ...
        for attempt in range(1, self._retries + 1):
            ...
            _LOGGER.warning(
                'CLI attempt %d/%d failed: %s', attempt, self._retries, last_error.strip()
            )
            time.sleep(self._backoff_seconds * attempt)
        raise RuntimeError(f'kaggle CLI failed after {self._retries} attempts: {last_error}')
    ...
    def create_dataset(self, slug: str, folder: str, title: str) -> None:
        ...
        with tempfile.TemporaryDirectory() as staging:
            ...
            with open(os.path.join(staged_folder, 'dataset-metadata.json'), 'w') as handle:
                json.dump(metadata, handle)
            self._run(['kaggle', 'datasets', 'create', '-p', staged_folder, '--dir-mode', 'zip'])
    ...
    def push_version(self, slug: str, folder: str) -> None:
        ...
        if not self.dataset_exists(slug):
            ...
            return
        self._run(['kaggle', 'datasets', 'version', '-p', folder, '--dir-mode', 'zip',
                   '-m', f'auto-version {int(time.time())}'])
