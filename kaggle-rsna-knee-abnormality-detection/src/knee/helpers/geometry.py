...
class SliceGeometry:
    ...
    def from_dataset(cls, dataset) -> 'SliceGeometry':
        ...
        return cls(
            ...
            position=tuple(float(x) for x in position) if position is not None else None,
            orientation=(
...
def order_slices(slices: list[SliceGeometry]) -> list[str]:
    ...
    if all(s.position is not None and s.orientation is not None for s in slices):
        orientations = np.stack([s.orientation for s in slices])  # type: ignore[misc]
...
def extract_spacing(first_slice: SliceGeometry, pixel_spacing: float | None) -> dict:
    ...
    return {
        ...
        'pixel_spacing': float(pixel_spacing) if pixel_spacing is not None else -1.0,
    }
