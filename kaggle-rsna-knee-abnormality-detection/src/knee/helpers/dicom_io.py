...
class DecoderRegistry:
  ...
  def __init__(self, backend_order: list[str]) -> None:
    ...
    self.backend_order = [
      name for name in backend_order if name in self._backends
    ]
  ...
  def decode_dataset(self, dataset: pydicom.dataset.Dataset) -> np.ndarray:
    ...
    separator = '; '
    raise ValueError(f'All decode backends failed -> {separator.join(errors)}')
  ...
  def read_slice(self, path: str) -> tuple[np.ndarray | None, dict]:
    ...
    try:
      ...
      _LOGGER.warning(
        'Unreadable slice %s (%s: %s)', path, type(exc).__name__, exc
      )
