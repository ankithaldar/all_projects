...
def scan_one_series(series_dir: str) -> dict | None:
  ...
  try:
    ...
    pixel_spacing = (
      float(np.mean([float(x) for x in spacing])) if spacing else -1.0
    )
    record = {
      'study': study_uid,
      'series': series_uid,
      ...
      'slice_thickness': float(
        getattr(first_meta, 'SliceThickness', -1.0) or -1.0
      ),
    ...
    # The scan must survive arbitrary per-file corruption; failures are
    # recorded and the series simply drops out of the index.
    _LOGGER.warning(
      'Failed scanning %s (%s: %s)', series_dir, type(exc).__name__, exc
    )
    return None
...
def build_index(dicom_root: str, workers: int) -> pd.DataFrame:
  ...
  frame['sop_uids'] = frame['sop_uids'].apply('|'.join)
  return frame
