...
TARGET_SPECS: tuple[TargetSpec, ...] = (
  TargetSpec(
    name='ACL', lexicon=r'\bacl\b|\banterior\s+cruciate(?:\s+ligament)?\b'
  ),
  TargetSpec(
    name='MCL', lexicon=r'\bmcl\b|\bmedial\s+collateral(?:\s+ligament)?\b'
  ),
  TargetSpec(
  ...
  TargetSpec(
    ...
    locator=(
      r'\bpatellofemoral\b'
      r'|\bpf\s+(?:compartment|joint|space)\b'
...
class RuleBasedLabeler:
  ...
  def _tokenize(text: str) -> _TokenizedReport:
    ...
    for match in _TOKEN_RE.finditer(lowered):
      ...
      while (
        cursor < len(sentence_ranges) and start >= sentence_ranges[cursor][1]
      ):
        cursor += 1
  ...
  def _classify_span(
    ...
    sentence = (
      report.sentence_ids[first] if first < len(report.sentence_ids) else -1
    )
  ...
  def _verdicts_for_pattern(
    self, report: _TokenizedReport, pattern: str
  ) -> list[int]:
  ...
  def _verdicts_cooccurrence(
    ...
    """
    mean_token_len = sum(e - s for s, e in report.spans) / max(
      len(report.spans), 1
    )
