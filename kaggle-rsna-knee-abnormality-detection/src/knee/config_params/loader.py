...
1. Dot-path overrides applied after loading, e.g.
   ``model.init_params.dropout=0.2``.
...
def _parse_scalar(value: str) -> Any:
  ...
  Returns:
        Parsed Python object (bool, int, float, list, dict) or the
        original string.
  """
  try:
...
def _resolve_interpolations(node: Any, root: dict) -> Any:
  ...
  if isinstance(node, dict):
    return {
      key: _resolve_interpolations(value, root) for key, value in node.items()
    }
...
def load_experiment(path: str, overrides: list[str] | None = None) -> dict:
  ...
  * ``override``: experiment-specific section applied last.
  ...
  for stem in experiment.get('defaults', []):
    with open(
      os.path.join(base_dir, f'{stem}.yaml'), 'r', encoding='utf-8'
    ) as handle:
