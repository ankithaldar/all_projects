...
def predict_studies(
    ...
    _, valid_dataset = build_datasets(
        config, index_df, labels_df=None,
        valid_study_ids=study_ids, train_study_ids=None,
    )
...
def write_submission(
    ...
    assert not frame.isna().any().any(), 'Missing predictions for some test studies'
    ...
    _LOGGER.info('Submission written: %s (%d rows)', submission_path, len(clipped))
