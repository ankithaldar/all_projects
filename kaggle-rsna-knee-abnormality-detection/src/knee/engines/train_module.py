...
class KneeModule(pl.LightningModule):
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        frame = pd.DataFrame(probs, columns=[f'{c}_prob' for c in self.target_columns])
        ...
        frame.to_csv(os.path.join(self.oof_dir, f'oof_fold{self.fold_id}.csv'), index=False)
    ...
    def configure_optimizers(self):
        ...
        remaining = [p for n, p in self.model.named_parameters()
        ...
        return {'optimizer': optimizer, 'lr_scheduler': schedulers[0]} if schedulers \
            else {'optimizer': optimizer}


def _resolve_class(class_path: str) -> type:
    """Import a class by dotted path (thin wrapper keeping imports lazy).
    ...
    """
    from knee.config_params.loader import instantiate  # noqa: F401
