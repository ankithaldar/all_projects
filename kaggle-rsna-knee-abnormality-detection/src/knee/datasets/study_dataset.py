...
def select_series(series_rows: pd.DataFrame, selection_cfg: dict) -> pd.DataFrame:
...
def build_metadata(
    ...
    flags = (
        [float(study_series['fluid_sensitive'].max()), float(study_series['fat_suppression'].max())]
        if len(study_series)
        else [0.0, 0.0]
    )
    counts = []
    ...
    if metadata_cfg.get('use_spacing_stats', True):
        ...
        spacing.append(float(column.mean()) if column is not None and column.notna().any() else -1.0)
        has_geometry = (
    ...
    return np.asarray(plane_onehot + sex_onehot + flags + counts + spacing, dtype=np.float32)
...
class StudyDataset(Dataset):
    ...
    def _load_series_stack(self, row: pd.Series) -> np.ndarray:
        ...
        if stack.shape[0] != self.n_slices:
            raise ValueError(
                f'Series {row["series"]} produced depth {stack.shape[0]} '
    ...
    def __getitem__(self, idx: int) -> dict[str, Any]:
        ...
        selected = select_series(group.reset_index(drop=True), self.series_selection)
        ...
        counts += [0] * pad
        meta_rows = np.zeros((self.n_series_tokens_max, SERIES_META_DIM), dtype=np.float32)
        for pos, (_, row) in enumerate(selected.iterrows()):
            plane_idx = {'Sagittal': 0, 'Coronal': 1, 'Axial': 2}.get(row.get('plane'), 0)
            meta_rows[pos] = (
                float(plane_idx),
