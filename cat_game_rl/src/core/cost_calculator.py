...
class CostCalculator:
  ...
  def total_cost(init_cost: int, batch_size: int) -> float:
    """Total cost for a batch.

    sum of init_cost*(1 + 0.5*(n-1)) for n=1..batch_size.
  ...
  def max_affordable_batch(init_cost: int, coins: int) -> int:
    ...
    b = max(0, int(b_float))
    while b > 0 and math.ceil(CostCalculator.total_cost(init_cost, b)) > coins:
      b -= 1
    if math.ceil(CostCalculator.total_cost(init_cost, b + 1)) <= coins:
      b += 1
    return b
