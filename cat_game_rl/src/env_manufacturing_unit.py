...
class ManufacturingUnit:
  ...
  def check_crafting_sufficient_coins(self):
    return self.item.game_economy.coins >= self.item.bom.calculate_batch_cost(self.batch_size)

  def check_input_item_availability(self):
    return self.item.game_economy.items_in_stash >= self.item.bom.calculate_batch_input(self.batch_size)

  # check if crafting end time is same as time in clock
  def check_crafting_end_time(self):
    return self.end_time == self.item.clock.current_time
  ...
  def start_crafting(self):
    ...
    self.item.game_economy.update_coins(
      used_coins=self.item.bom.calculate_batch_cost(self.batch_size)
    )

    # update crafted account stash
    self.item.game_economy.update_stash(
      used_stash=self.item.bom.calculate_batch_input(self.batch_size)
    )
    ...
    self.start_time = self.item.clock.current_time
